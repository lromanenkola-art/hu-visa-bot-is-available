...
def wait_for_real_outcome(page, timeout_ms=25000):
    ...
    ДВУХ подтверждённых исходов:
      - "no_slots"  -> появилась видимая красная надпись "нет мест" (#nocase
                       или запасной способ текст+кнопка Rendben)
      - "calendar"  -> реально открылся календарь (шаг 2)

    Если за отведённое время ни один из строгих способов не сработал -
    делаем ПОСЛЕДНЮЮ проверку по явному требованию: если сама красная
    фраза "nincs szabad időpont" вообще присутствует на странице (даже
    если её видимость не удалось строго подтвердить) - считаем, что
    слотов нет, и НЕ поднимаем ошибку. Эта фраза - специфичное системное
    сообщение сайта, которое не встречается больше нигде, поэтому её
    наличие само по себе достаточно надёжный сигнал.

    Если и этой фразы нет вовсе - тогда это действительно "timeout",
    то есть ошибка (не удалось подтвердить ни один из исходов).
    """
    waited = 0
    step_ms = 500
    ...
    try:
        ...
        if "nincs szabad időpont" in content:
            print(
                "Финальная проверка: фраза 'nincs szabad időpont' присутствует "
                "на странице - считаем, что слотов нет (даже если видимость "
                "не подтвердилась строгими способами)"
            )
            safe_screenshot(page, "step5_red_nocase_message_final_check.png")
            return "no_slots"
    except Exception as e:
        print("Ошибка при финальной проверке содержимого страницы: " + str(e))
